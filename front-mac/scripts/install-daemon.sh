#!/usr/bin/env bash
# Install / reinstall the LaunchAgent that runs the mac-gpu-monitord backend
...
LABEL="com.maximofn.mac-gpu-monitord"
SRC="$(cd "$(dirname "$0")" && pwd)/${LABEL}.plist"
DST="$HOME/Library/LaunchAgents/${LABEL}.plist"
BIN="$(cd "$(dirname "$0")/../.." && pwd)/target/release/mac-gpu-monitord"
...
case "$cmd" in
    install)
        ...
        echo
        echo "Loaded. The backend will autostart on login and listen on http://127.0.0.1:9133."
        echo "Logs: ~/Library/Logs/mac-gpu-monitord.{out,err}.log"
        ;;
    uninstall)
        if launchctl print "$target" >/dev/null 2>&1; then
            echo "==> bootout $target"
