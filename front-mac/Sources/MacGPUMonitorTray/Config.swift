...
struct Config: Sendable {
    ...
    static let `default` = Config(
        backendURL: ProcessInfo.processInfo.environment["MAC_GPU_MONITOR_TRAY_URL"]
            ?? "http://127.0.0.1:9133",
        iconHeight: 22,
        logLevel: "info",
        dumpIcon: nil
    ...
    static func parse(_ args: [String]) -> Config {
        ...
        while i < args.count {
            ...
            case "-h", "--help":
                ...
                exit(0)
            case "--version":
                print("mac-gpu-monitor-tray 0.1.0")
                exit(0)
            default:
    ...
    static func printUsage() {
        let usage = """
        mac-gpu-monitor-tray — macOS menu bar frontend for mac-gpu-monitord

        USAGE:
            mac-gpu-monitor-tray [OPTIONS]

        OPTIONS:
            --backend-url <URL>     Base URL of mac-gpu-monitord (default: http://127.0.0.1:9133,
                                    env: MAC_GPU_MONITOR_TRAY_URL)
