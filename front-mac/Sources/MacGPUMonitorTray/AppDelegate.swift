...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let logger = Logger(subsystem: "com.maximofn.mac-gpu-monitor", category: "app")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // AsyncStream — Swift 6 forbids capturing a @MainActor controller from
        // inside the @Sendable client closure.
    ...
    private func handle(update: ClientUpdate) {
        ...
        case .connected(let snap):
            let pct = snap.gpus.first?.utilization.gpuPercent ?? 0
            logger.debug("snapshot: gpu=\(pct, privacy: .public)%")
